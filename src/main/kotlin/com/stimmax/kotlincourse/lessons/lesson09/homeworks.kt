...
fun main() {
    ...
    println()
    println(
        "Имея изменяемый список чисел, измените значение элемента " +
    ...
    println(max)
    println()
    println("// Имея список целых чисел, " +
            "создайте новый список, " +
            "содержащий только четные числа из исходного списка используя цикл.")
    val numlist1: List<Int> = listOf(1,2,3,4,5,6)
    val newnumlist1 = mutableListOf<Int>()
    for (num in numlist1) {
        if (num % 2 == 0) {
            newnumlist1.add(num)
        }
    ...
    println(newnumlist1)
    println()
    println("Создайте пустое неизменяемое множество целых чисел.")
    val setint: Set<Int> = setOf()
    println(setint)
    println()
    println("Создайте неизменяемое множество целых чисел, " +
            "содержащее три различных элемента (например, 1, 2, 3).")
    val setint1: Set<Int> = setOf(1,2,3)
    println(setint1)
    println()
    println("Создайте изменяемое множество строк и " +
            "инициализируйте его несколькими значениями (например, Kotlin, Java, Scala).")
    val setint1mut: MutableSet<String> = mutableSetOf("Kotlin", "Java", "Scala")
    println(setint1mut)
    println()
    println("Имея изменяемое множество строк, добавьте " +
            "в него новые элементы (например, Swift, Go).")
    val setint2mut: MutableSet<String> = mutableSetOf("Kotlin", "Java", "Scala")
    setint2mut.add("Swift")
    setint2mut.add("Go")
    println(setint2mut)
    println()
    println("Имея изменяемое множество целых чисел, " +
            "удалите из него определенный элемент (например, 2).")
    val setnummut: MutableSet<Int> = mutableSetOf(1,2,6,8)
    setnummut.remove(2)
    println(setnummut)
    println()
    println("Создайте множество целых чисел и используйте " +
            "цикл для вывода каждого элемента на экран.")
    val setnum1: Set<Int> = setOf(1,2,3,4)
    for (num in setnum1) {println(num)}
    println()
    println("Создай функцию, которая принимает множество строк (set) и строку и проверяет, " +
